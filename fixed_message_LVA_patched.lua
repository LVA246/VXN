...
if LPH_OBFUSCATED == nil then
    ...
    LPH_ENCNUM = function(toEncrypt, ...)
        assert(type(toEncrypt) == "number" and select('#', ...) == 0, "LPH_ENCNUM only accepts a single constant double or integer as an argument.")
        return toEncrypt
    end
    LPH_NUMENC = LPH_ENCNUM
    LPH_ENCSTR = function(toEncrypt, ...)
        assert(type(toEncrypt) == "string" and select('#', ...) == 0, "LPH_ENCSTR only accepts a single constant string as an argument.")
        return toEncrypt
    end
    LPH_STRENC = LPH_ENCSTR
    LPH_ENCFUNC = function(toEncrypt, encKey, decKey, ...)
        
        assert(type(toEncrypt) == "function" and type(encKey) == "string" and select('#', ...) == 0, "LPH_ENCFUNC accepts a constant function, constant string, and string variable as arguments.")
        return toEncrypt
    end
    LPH_FUNCENC = LPH_ENCFUNC
    LPH_JIT = function(f, ...)
        assert(type(f) == "function" and select('#', ...) == 0, "LPH_JIT only accepts a single constant function as an argument.")
        return f
    end
    LPH_JIT_MAX = LPH_JIT
    LPH_NO_VIRTUALIZE = function(f, ...)
        assert(type(f) == "function" and select('#', ...) == 0, "LPH_NO_VIRTUALIZE only accepts a single constant function as an argument.")
        return f
    end
    LPH_NO_UPVALUES = function(f, ...)
        assert(type(setfenv) == "function", "LPH_NO_UPVALUES can only be used on Lua versions with getfenv & setfenv")
        assert(type(f) == "function" and select('#', ...) == 0, "LPH_NO_UPVALUES only accepts a single constant function as an argument.")
        local env = getrenv()
        return setfenv(
...
local Game_Name = (game.PlaceId == 4991214437 and "Town") or (game.PlaceId == 18642421777 and "The LVA") or (game.PlaceId == 16472538603 and "The LVA") or (game.PlaceId == 13643807539 and "South LVA") or (game.PlaceId == 93612682780562 and "South LVA") or (game.PlaceId == 14413475235 and "South LVA") or (game.PlaceId == 104715542330896 and "BlockSpin") or (game.PlaceId == 71600459831333 and "Street Life") or "Universal" or game.PlaceId == 13643807539
...
local Services = setmetatable({}, {
    __index = LPH_NO_VIRTUALIZE(function(self, service, key)
        if Solara and (service == "VirtualInputManager") and (Game_Name == "The LVA") then
...
if Game_Name == "South LVA" and not Solara then
...
if script_key then
    writefile("LVA_Key.txt", script_key)
end
...
if isfunctionhooked then
    if isfunctionhooked(FireServer) or isfunctionhooked(UnreliableFireServer) or isfunctionhooked(InvokeServer) and LPH_OBFUSCATED then
        return Services.LocalPlayer:Kick("LVA | Security : You are running another script, please disable it and execute again")
    end
end
...
local Config = {
    ["TheLVA"] = {
        ...
        ["TeleportationList"] = {
            ...
            ["Capital One Bank 🏦"] = CFrame.new(-223, 284, -1200);
            ...
            ["Hospital🚑"] = CFrame.new(-1590, 254, 17);
             };
    ...
    ["Game"] = {
        ["Ray_Systems"] = (Game_Name == "Road To Riches" and {"Raycast"} or Game_Name == "BlockSpin" and {"Raycast"} or Game_Name == "Town" and {"Raycast"} or Game_Name == "Universal" and {"Raycast","FindPartOnRay","FindPartOnRayWithWhitelist"} or Game_Name == "The LVA" and {"FindPartOnRay"}  or Game_Name == "South LVA" and {"Raycast"} or Game_Name == "Street Life" and {"Raycast"} or Game_Name == "Criminality" and {"Raycast"}) or {};
        ["Wall_Bang_Possible"] = ((Game_Name == "Universal" or Game_Name == "Criminality" or Game_Name == "South LVA" or Game_Name == "The LVA" or Game_Name == "Street Life"));
    };
    ...
    ESP = {
        ...
        Options = { 
            ...
            Highlight = false, HighlightRGB = Color3.fromRGB(255, 215, 0),
        },
        Drawing = {
            Chams = {
                Enabled  = false,
                Thermal = true,
                FillRGB = Color3.fromRGB(255, 215, 0),
            ...
            Weapons = {
                Enabled = false, WeaponTextRGB = Color3.fromRGB(255, 215, 0),
                Outlined = false,
                Gradient = false,
                Transparency = 0,
                GradientRGB1 = Color3.fromRGB(255, 255, 255), GradientRGB2 = Color3.fromRGB(255, 215, 0),
            },
            Inventory = {
            ...
            Healthbar = {
                ...
                Gradient = true, GradientRGB1 = Color3.fromRGB(255, 215, 0), GradientRGB2 = Color3.fromRGB(0,255,0)
            },
            Boxes = {
                Animate = true,
                RotationSpeed = 300,
                Gradient = false, GradientRGB1 = Color3.fromRGB(255, 215, 0), GradientRGB2 = Color3.fromRGB(0, 0, 0), 
                GradientFill = true, GradientFillRGB1 = Color3.fromRGB(255, 215, 0), GradientFillRGB2 = Color3.fromRGB(0, 0, 0), 
                Filled = {
                    Enabled = false,
                    Transparency = 0.75,
...
--[[if not Solara and Game_Name == "The LVA" then
    local DTC;
...
if not Solara then
    if Game_Name == "BlockSpin" then
		...
		for _, v in pairs(Required) do
			if not getgenv()[v] then
				game:GetService("Players").LocalPlayer:Kick(string.format("Your executor does not support [%s], which is REQUIRED to use the BlockSpin script.", v))
			end
		end
		...
		for _, Signal in pairs(BlacklistedSignals) do
		...
		OldIndex = hookmetamethod(game, "__index", function(Self, Key)
			...
			for _, BlacklistedSignal in pairs(BlacklistedSignals) do
    ...
    if Game_Name == "Universal" or Game_Name == "Road To Riches" or Game_Name == "Street Life" or Game_Name == "The LVA" or Game_Name == "Town" and not Solara then
        local DTC;
    ...
    LPH_JIT_MAX(function()
        ...
        if Solara or not fireproximityprompt or string.find(identifyexecutor(), "MacSploit") then
            getgenv().fireproximityprompt = LPH_NO_VIRTUALIZE(function(self, vuln)
                ...
                if not vuln then
                    ...
                    for Index, Value in pairs(prompt_settings) do
                        self[Index] = Value
                    end
                else
                    self.HoldDuration = 0; self.RequiresLineOfSight = false;
        ...
        if Game_Name == "The LVA" then
            RunService:BindToRenderStep("MiscSettings", 1000 , LPH_NO_VIRTUALIZE(function(Delta)
                ...
                if not (Game_Name == "South LVA") and not (Game_Name == "Road To Riches") then
                ...
                if Config.TheLVA.PlayerUtilities.SpectatePlayer then
                    Set_Spectate = false
                    local Subject = Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer) and Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character and Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("Humanoid")

                    if not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer) or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("Humanoid") then
            ...
            RunService:BindToRenderStep("MiscSettings", 1000 , LPH_NO_VIRTUALIZE(function()
                ...
                if Config.South_LVA.PlayerUtilities.SpectatePlayer then
                    Set_Spectate = false
                    local Subject = Players:FindFirstChild(Config.South_LVA.PlayerUtilities.SelectedPlayer) and Players:FindFirstChild(Config.South_LVA.PlayerUtilities.SelectedPlayer).Character and Players:FindFirstChild(Config.South_LVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("Humanoid")

                    if not Players:FindFirstChild(Config.South_LVA.PlayerUtilities.SelectedPlayer) or not Players:FindFirstChild(Config.South_LVA.PlayerUtilities.SelectedPlayer).Character or not Players:FindFirstChild(Config.South_LVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("Humanoid") then
                        Subject = LocalPlayer.Character.Humanoid
                    end
                ...
                if Game_Name == "South LVA" then
                    if LocalPlayer.Character.Head:FindFirstChild("RankTag") then
                        --if not getgenv().Window then return end

                        LocalPlayer.Character.Head:FindFirstChild("RankTag").MainFrame.NameLabel.Visible = not Config.South_LVA.LocalPlayer_Config.HideName
                    end
                end

                if Game_Name == "South LVA" and Solara then
                    return
                end

                if Game_Name == "South LVA" and Config.South_LVA.LocalPlayer_Config.InfiniteStamina and not Solara then
        ...
        GetClosestPlayerToMouseAimbot = LPH_NO_VIRTUALIZE(function()
            ...
            if library.priority[1] then
                for Index, Value in pairs(Plrs) do
                    ...
                    if (Value.Character and Value.Character:FindFirstChild(Config.Aimlock.TargetPart) and Value.Character:FindFirstChild("Humanoid") and Value.Character:FindFirstChild("Humanoid").Health > (Game_Name == "South LVA" and 4 or 0)) then
        ...
        GetClosestPlayerToMouseSilent = LPH_NO_VIRTUALIZE(function()
            ...
            if library.priority[1] then
                for Index, Value in pairs(Plrs) do
                    ...
                    if not table.find(library.priority, Value.Name) then -- continue end;
                    if table.find(library.whitelist, Value.Name) then -- continue end;
                    if (Value.Character and FindFirstChild(Value.Character, "HumanoidRootPart") and FindFirstChild(Value.Character,"Humanoid") and FindFirstChild(Value.Character,"Humanoid").Health > (Game_Name == "South LVA" and 4 or 0)) then
                        if FindFirstChildOfClass(Value.Character, "ForceField") then -- continue end
                        if FindFirstChild(Value.Character, "Torso") and FindFirstChild(Value.Character, "Torso").Material == Enum.Material.ForceField then -- continue end
                        if Value.Character.Parent == nil then -- continue end
            ...
            if Method == "Raycast" and table.find(Config.Game.Ray_Systems, "Raycast") then
                ...
                if Config.Silent.Enabled then
                    if Config.Silent.Targetting then
                        ...
                        if not Config.Silent.WallBang and Game_Name == "South LVA" then
                            local RaycastParams = RaycastParams.new()
        ...
        for Index, Value in Players:GetPlayers() do
            if Value == LocalPlayer then -- continue end;
            if Game_Name == "South LVA" then -- continue end
        ...
        Players.PlayerAdded:Connect(function(Value)
            if Game_Name == "South LVA" then return end
        ...
        if Game_Name == "South LVA" then
            if not Solara then
                ...
                local ModWeapon = LPH_JIT_MAX(function(Weapon)
                    ...
                    task.spawn(function()
                        local Success, Error = pcall(LPH_NO_VIRTUALIZE(function()
                            ...
                            --[[if Config.South_LVA._Modifications.InfiniteAmmo then
                                Module.Mag = Config.South_LVA._Modifications.InfiniteAmmo and math.huge or OldConfig.Ammo
            
                                SetInfiniteAmmo = false
                            end
            
                            if Config.South_LVA._Modifications.InfiniteAmmo == false and SetInfiniteAmmo == false then
                                Module.Mag = OldConfig.MaxAmmo / 2
                                SetInfiniteAmmo = true
                            end]]
            
                            --[[if Config.South_LVA._Modifications.InfiniteClips then
                                Module.Ammo = Config.South_LVA._Modifications.InfiniteClips and 999999 or OldConfig.MaxAmmo
            
                                SetInfiniteClips = false
                            end]]
            
                            if Config.South_LVA._Modifications.InfiniteClips == false and SetInfiniteClips == false then
                                Module.Ammo = OldConfig.MaxAmmo
                                SetInfiniteClips = true
                            end
            
                            if ModuleSettings.LimitedAmmoEnabled then
                                ModuleSettings.LimitedAmmoEnabled = Config.South_LVA._Modifications.InfiniteAmmo and false or OldConfig.LimitedAmmoEnabled
                            end
                            ...
                            task.spawn(function()
                                while Weapon ~= nil and LocalPlayer.Character ~= nil and Weapon.Parent == LocalPlayer.Character and Module ~= nil and ModuleSettings ~= nil and ModuleSettings.FireRate ~= nil and task.wait(.1) do
                                    if Config.South_LVA._Modifications.DisableJamming then
                            ...
                            ModuleSettings.FireRate = Config.South_LVA._Modifications.ModifyFireRate and GetPercentage(OldConfig.FireRate, Config.South_LVA._Modifications.FireRateSpeed) or OldConfig.FireRate
                            
                            ModuleSettings.ReloadTime = Config.South_LVA._Modifications.ModifyReloadSpeed and GetPercentage(OldConfig.ReloadTime, Config.South_LVA._Modifications.ReloadSpeed) or OldConfig.ReloadTime
                            ModuleSettings.TacticalReloadTime = Config.South_LVA._Modifications.ModifyReloadSpeed and GetPercentage(OldConfig.TacticalReloadTime, Config.South_LVA._Modifications.ReloadSpeed) or OldConfig.TacticalReloadTime
                            
                            ModuleSettings.EquippingTime = Config.South_LVA._Modifications.ModifyEquipSpeed and GetPercentage(OldConfig.EquippingTime, Config.South_LVA._Modifications.EquipSpeed) or OldConfig.EquippingTime
                            
                            ModuleSettings.SpreadX = Config.South_LVA._Modifications.ModifySpreadValue and GetPercentage(OldConfig.SpreadX, Config.South_LVA._Modifications.SpreadPercentage) or OldConfig.SpreadX
                            ModuleSettings.SpreadY = Config.South_LVA._Modifications.ModifySpreadValue and GetPercentage(OldConfig.SpreadY, Config.South_LVA._Modifications.SpreadPercentage) or OldConfig.SpreadY
            
                            ModuleSettings.Recoil = Config.South_LVA._Modifications.ModifyRecoilValue and GetPercentage(OldConfig.Recoil, Config.South_LVA._Modifications.RecoilPercentage) or OldConfig.Recoil
            
                            ModuleSettings.ShotgunEnabled = Config.South_LVA._Modifications.InstantKill and true or OldConfig.ShotgunEnabled
            
                            ModuleSettings.Auto = Config.South_LVA._Modifications.Automatic or OldConfig.Auto
                        
                            ModuleSettings.JamChance = Config.South_LVA._Modifications.DisableJamming and 0 or OldConfig.JamChance
                        end))
                ...
                local ConfigMet7atable = getmetatable(Config.South_LVA.Modifications)

                ConfigMetatable.__index = LPH_NO_VIRTUALIZE(function(.....)
                    return Config.South_LVA._Modifications[select(2, ...)]
                end)
        
                ConfigMetatable.__newindex = LPH_NO_VIRTUALIZE(function(.....)
                    local Index, Value = select(2, ...)
        
                    Config.South_LVA._Modifications[Index] = Value; ModWeapons()
                end)
                ...
                local OldMagAndAmmo; OldMagAndAmmo = hookmetamethod(game, "__namecall", LPH_NO_VIRTUALIZE(function(Self, ...)
                    ...
                    if getnamecallmethod() == "FireServer" or getnamecallmethod() == "InvokeServer" and tostring(Self) == "ChangeMagAndAmmo" and (Config.South_LVA._Modifications.InfiniteAmmo or Config.South_LVA._Modifications.InfiniteClips) then
                        local Arguments = {...}
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while task.wait(0.1) do
                    if not Config.South_LVA.KillAura then -- continue end
                    ...
                    for Index, Value in Players:GetPlayers() do
                        if table.find(Config.South_LVA.KillAuraWhitelist, tostring(Value)) then -- continue end
                        if Value == LocalPlayer then -- continue end
                        ...
                        if not DistanceCheck(Value, Config.South_LVA.KillAuraRange) then -- continue end
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do
                    ...
                    if not Config.South_LVA.LocalPlayer_Config.Speed then -- continue end
                    ...
                    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
                        ...
                        if Humanoid.MoveDirection.Magnitude > 0 then
                            ...
                            LocalPlayer.Character:TranslateBy(
                                Humanoid.MoveDirection * Config.South_LVA.LocalPlayer_Config.SpeedValue 
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while task.wait() do
                    if not Config.South_LVA.PlayerUtilities.BringingPlayer then -- continue end
                    if tostring(Config.South_LVA.PlayerUtilities.SelectedPlayer) == tostring(LocalPlayer) then -- continue end
                    if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") or not Players:FindFirstChild(Config.South_LVA.PlayerUtilities.SelectedPlayer) or not Players:FindFirstChild(Config.South_LVA.PlayerUtilities.SelectedPlayer).Character or not Players:FindFirstChild(Config.South_LVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("HumanoidRootPart") then -- continue end
                    Players:FindFirstChild(Config.South_LVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("HumanoidRootPart").CFrame = LocalPlayer.Character:FindFirstChild("HumanoidRootPart").CFrame + Vector3.new(2, 0, 0)
                end
            end))
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do
                    task.wait(0)
                    if Config.South_LVA.VehicleModifications.SpeedEnabled and UserInputService:IsKeyDown(Enum.KeyCode.W) then
                        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
                            if LocalPlayer.Character and typeof(LocalPlayer.Character) == "Instance" then
                                ...
                                if Humanoid and typeof(Humanoid) == "Instance" then
                                    ...
                                    if SeatPart and typeof(SeatPart) == "Instance" and SeatPart:IsA("VehicleSeat") then
                                        SeatPart.AssemblyLinearVelocity *= Vector3.new(1 + Config.South_LVA.VehicleModifications.SpeedValue, 1, 1 + Config.South_LVA.VehicleModifications.SpeedValue)
                                    end
                                end
                            end
                        end
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do
                    task.wait(0)
                    if Config.South_LVA.VehicleModifications.BreakEnabled and UserInputService:IsKeyDown(Enum.KeyCode.S) then
                        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
                            if LocalPlayer.Character and typeof(LocalPlayer.Character) == "Instance" then
                                local Humanoid = LocalPlayer.Character:FindFirstChildWhichIsA("Humanoid")
                                if Humanoid and typeof(Humanoid) == "Instance" then
                                    local SeatPart = Humanoid.SeatPart
                                    if SeatPart and typeof(SeatPart) == "Instance" and SeatPart:IsA("VehicleSeat") then
                                        SeatPart.AssemblyLinearVelocity *= Vector3.new(1 - Config.South_LVA.VehicleModifications.BreakValue, 1, 1 - Config.South_LVA.VehicleModifications.BreakValue)
                                    end
                                end
                            end
                        end
            ...
            UserInputService.InputBegan:Connect(function(Input, GameProcessedEvent)
                if Input.KeyCode == Config.South_LVA.VehicleModifications.InstantStopBind and Config.South_LVA.VehicleModifications.InstantStop and (not GameProcessedEvent) then
                    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
                        if LocalPlayer.Character and typeof(LocalPlayer.Character) == "Instance" then
                            local Humanoid = LocalPlayer.Character:FindFirstChildWhichIsA("Humanoid")
                            if Humanoid and typeof(Humanoid) == "Instance" then
                ...
                if Config.South_LVA.LocalPlayer_Config.DeleteKey ~= nil and Input.UserInputType == Enum.UserInputType.MouseButton1 and UserInputService:IsKeyDown(Config.South_LVA.LocalPlayer_Config.DeleteKey) and Config.South_LVA.LocalPlayer_Config.DeleteOnKey and (not GameProcessedEvent) then
            ...
            ProximityPromptService.PromptButtonHoldBegan:Connect(LPH_NO_VIRTUALIZE(function(Prompt, Player)
                if Prompt and Player == LocalPlayer and fireproximityprompt then
                    if Config.South_LVA.LocalPlayer_Config.InstantInteract then
                    ...
                    task.spawn(function()
                        ...
                        while tick() < endTime do
                            ...
                            textLabel.Text = string.format(
                                'LVA\n%s\nplease wait : <font color="rgb(255,215,0)">%.2f</font> seconds',
            ...
            getgenv().Teleport = LPH_NO_VIRTUALIZE(function(CFrame)   
                if Config.South_LVA.TeleportMethod == 'Dirt Bike' then
                    ...
                    return "Success"
                elseif Config.South_LVA.TeleportMethod == 'Tween' then
                ...
                elseif Config.South_LVA.TeleportMethod == 'Damage' then
            ...
            Start_BoxFarm = function()
                boxfarm_thread = task.spawn(LPH_NO_VIRTUALIZE(function()
                    while task.wait() do
                        ...
                        if not Config.South_LVA.FarmingUtilities.BoxFarm then -- continue end
            ...
            task.spawn(LPH_JIT_MAX(function()
                while task.wait(.1) do
                    if not Config.South_LVA.FarmingUtilities.CardFarm then -- continue end
            ...
            task.spawn(LPH_JIT_MAX(function()
                while task.wait(.1) do
                    if not Config.South_LVA.FarmingUtilities.MarshmallowFarm then -- continue end
                    if not LocalPlayer.Character and not LocalPlayer.Character:FindFirstChild("Humanoid") then -- continue end
            ...
            Start_MarshmallowFarm = function()
                MarshmallowFarm_Thread = task.spawn(LPH_JIT_MAX(function()
                    while wait() do
                        if not Config.South_LVA.FarmingUtilities.MarshmallowFarm then -- continue end
                        local Owns_Bike = Config.South_LVA.OwnedBike == "No"
                        
                        local Marshmellow_Increment = Config.South_LVA.FarmingUtilities.MarshmallowIncrement
            ...
            Start_ChipFarm = function()
                ChipFarm_Thread = task.spawn(LPH_JIT_MAX(function()
                    while task.wait(1) do
                        if not Config.South_LVA.FarmingUtilities.ChipFarm then -- continue end
            ...
            Start_CardFarm = function()
                CardFarm_Thread = task.spawn(LPH_JIT_MAX(function()
                    while task.wait(1) do
                        ...
                        if Config.South_LVA.TeleportMethod ~= "Tween" then
            ...
            Humanoid.Died:Connect(function()
                ...
                if Config.South_LVA.FarmingUtilities.CardFarm then
                    Stop_CardFarm()
                end

                if Config.South_LVA.FarmingUtilities.BoxFarm then
                    Stop_BoxFarm()
                end

                if Config.South_LVA.FarmingUtilities.ChipFarm then
                    Stop_ChipFarm()
                end

                if Config.South_LVA.FarmingUtilities.MarshmallowFarm then
            ...
            if Workspace:FindFirstChild("PromptPurchases") then
                ...
                for Index, Value in Workspace.PromptPurchases:GetChildren() do
                    ...
                    if not table.find(Config.South_LVA.Guns, GunWithPrice) then
                        table.insert(Config.South_LVA.Guns, GunWithPrice)

                        GunPosition[Value.Name] = Value.proxprompt.CFrame
                    end
                end

                table.sort(Config.South_LVA.Guns)
            end
        end
        ...
        if Game_Name == "The LVA" and RequireSupport then                                                
            local OldWeaponValues = {}
            ...
            local ModWeapon = LPH_JIT_MAX(function(Weapon)
                ...
                if Config.TheLVA._Modifications.InfiniteClips then
                    debug.setupvalue(getsenv(Weapon:FindFirstChild("GunScript_Local")).Reload, 3, 9e17)

                    SetInfiniteClips = false
                end

                if Config.TheLVA._Modifications.InfiniteClips == false and SetInfiniteClips == false then
                    debug.setupvalue(getsenv(Weapon:FindFirstChild("GunScript_Local")).Reload, 3, OldConfig.AmmoPerMag)

                    SetInfiniteClips = true
                end

                if Config.TheLVA._Modifications.InfiniteAmmo then
                    debug.setupvalue(getsenv(Weapon:FindFirstChild("GunScript_Local")).Reload, 5, Config.TheLVA._Modifications.InfiniteAmmo and 9e17 or OldConfig.AmmoPerMag / 2)
                    SetInfiniteAmmo = false
                end

                if Config.TheLVA._Modifications.InfiniteAmmo == false and SetInfiniteAmmo == false then
                ...
                Module.FireRate = Config.TheLVA._Modifications.ModifyFireRate and GetPercentage(OldConfig.FireRate, Config.TheLVA._Modifications.FireRateSpeed) or OldConfig.FireRate
                            
                Module.ReloadTime = Config.TheLVA._Modifications.ModifyReloadSpeed and GetPercentage(OldConfig.ReloadTime, Config.TheLVA._Modifications.ReloadSpeed) or OldConfig.ReloadTime
                                
                if Module.SpreadXY then
                    Module.SpreadXY = Config.TheLVA._Modifications.ModifySpreadValue and GetPercentage(OldConfig.SpreadXY, Config.TheLVA._Modifications.SpreadPercentage) or OldConfig.SpreadXY
                end

                if Module.SpreadYX then
                    Module.SpreadYX = Config.TheLVA._Modifications.ModifySpreadValue and GetPercentage(OldConfig.SpreadYX, Config.TheLVA._Modifications.SpreadPercentage) or OldConfig.SpreadYX
                end

                if Module.Spread then
                    Module.Spread = Config.TheLVA._Modifications.ModifySpreadValue and GetPercentage(OldConfig.Spread, Config.TheLVA._Modifications.SpreadPercentage) or OldConfig.Spread
                end

                Module.SpreadX = Config.TheLVA._Modifications.ModifySpreadValue and GetPercentage(OldConfig.SpreadX, Config.TheLVA._Modifications.SpreadPercentage) or OldConfig.SpreadX
                Module.SpreadY = Config.TheLVA._Modifications.ModifySpreadValue and GetPercentage(OldConfig.SpreadY, Config.TheLVA._Modifications.SpreadPercentage) or OldConfig.SpreadY

                Module.Recoil = Config.TheLVA._Modifications.ModifyRecoilValue and GetPercentage(OldConfig.Recoil, Config.TheLVA._Modifications.RecoilPercentage) or OldConfig.Recoil

                Module.BaseDamage = Config.TheLVA._Modifications.InfiniteDamage and math.huge or OldConfig.BaseDamage

                Module.Auto = Config.TheLVA._Modifications.Automatic or OldConfig.Auto
            
                Module.JamChance = Config.TheLVA._Modifications.DisableJamming and 0 or OldConfig.JamChance
    
                Module.Auto = Config.TheLVA._Modifications.Automatic or OldConfig.Auto
        
                Module.EquipTime = Config.TheLVA._Modifications.ModifyEquipSpeed and GetPercentage(OldConfig.EquipTime, Config.TheLVA._Modifications.EquipSpeed) or OldConfig.EquipTime

                Module.JamChance = Config.TheLVA._Modifications.NoJam and 0 or OldConfig.JamChance
            end)
            ...
            local ConfigMetatable = getmetatable(Config.TheLVA.Modifications)

            ConfigMetatable.__index = LPH_NO_VIRTUALIZE(function(.....)
                return Config.TheLVA._Modifications[select(2, ...)]
            end)
    
            ConfigMetatable.__newindex = LPH_NO_VIRTUALIZE(function(.....)
                local Index, Value = select(2, ...)
    
                Config.TheLVA._Modifications[Index] = Value; ModWeapons()
            end)
        end
        ...
        if Game_Name == "The LVA" then
            RunService.RenderStepped:Connect(LPH_NO_VIRTUALIZE(function()
                if LocalPlayer.PlayerGui:FindFirstChild("Run") and LocalPlayer.PlayerGui.Run:FindFirstChild("StaminaBarScript", true) then
                    LocalPlayer.PlayerGui.Run:FindFirstChild("StaminaBarScript", true).Disabled = Config.TheLVA.PlayerModifications.InfiniteStamina
                end

                if LocalPlayer.PlayerGui:FindFirstChild("Hunger") and LocalPlayer.PlayerGui.Hunger:FindFirstChild("HungerBarScript", true) then
                    LocalPlayer.PlayerGui.Hunger:FindFirstChild("HungerBarScript", true).Disabled = Config.TheLVA.PlayerModifications.InfiniteHunger
                end

                if LocalPlayer.PlayerGui:FindFirstChild("SleepGui") and LocalPlayer.PlayerGui.SleepGui:FindFirstChild("sleepScript", true) then
                    LocalPlayer.PlayerGui.SleepGui:FindFirstChild("sleepScript", true).Disabled = Config.TheLVA.PlayerModifications.InfiniteSleep
                end

                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("CameraBobbing") then
                    LocalPlayer.Character:FindFirstChild("CameraBobbing").Disabled = Config.TheLVA.PlayerModifications.DisableCameraBobbing
                end

                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("FallDamageRagdoll") then
                    LocalPlayer.Character:FindFirstChild("FallDamageRagdoll").Disabled = Config.TheLVA.PlayerModifications.NoFallDamage
                end

                if LocalPlayer.PlayerGui:FindFirstChild("BloodGui") then
                    LocalPlayer.PlayerGui:FindFirstChild("BloodGui").Enabled = not Config.TheLVA.PlayerModifications["DisableBloodEffects"]
                end
                ...
                if LocalPlayer.PlayerGui:FindFirstChild("JumpDebounce") and LocalPlayer.PlayerGui:FindFirstChild("JumpDebounce"):FindFirstChild("LocalScript") then
                    LocalPlayer.PlayerGui:FindFirstChild("JumpDebounce").LocalScript.Disabled = Config.TheLVA.PlayerModifications.NoJumpCooldown
                end
                ...
                if LocalPlayer.PlayerGui:FindFirstChild("CameraTexts") and LocalPlayer.PlayerGui:FindFirstChild("CameraTexts"):FindFirstChild("LocalScript") then
                    LocalPlayer.PlayerGui:FindFirstChild("CameraTexts").Enabled = not Config.TheLVA.PlayerModifications.DisableCameras
                    LocalPlayer.PlayerGui:FindFirstChild("CameraTexts").LocalScript.Disabled = Config.TheLVA.PlayerModifications.DisableCameras
                end
                ...
                if LocalPlayer.PlayerGui:FindFirstChild("RentGui") and LocalPlayer.PlayerGui:FindFirstChild("RentGui"):FindFirstChild("LocalScript") then
                    LocalPlayer.PlayerGui:FindFirstChild("RentGui").LocalScript.Disabled = Config.TheLVA.PlayerModifications.NoRentPay
                end

                if Config.TheLVA.PlayerModifications.AutoPickupBags and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if Config.TheLVA.PlayerModifications.AutoPickupCash and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            ...
            LocalPlayer.CharacterAdded:Connect(LPH_NO_VIRTUALIZE(function()
                if Config.TheLVA.PlayerModifications.DisableCameras then
            ...
            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
                ...
                LocalPlayer.Character.DescendantAdded:Connect(function(Descendant)
                    if Descendant:IsA("BodyVelocity") or Descendant:IsA("LinearVelocity") or Descendant:IsA("VectorForce") and Config.TheLVA.PlayerModifications.NoKnockback then
            ...
            LocalPlayer.CharacterAdded:Connect(LPH_NO_VIRTUALIZE(function(Character)
                ...
                if Config.TheLVA.PlayerModifications.RespawnWhereYouDied and typeof(DeathFrame) == "CFrame" then
            ...
            ResetFlight = LPH_NO_VIRTUALIZE(function()
                for i, v in pairs(flight_registry) do
                    local t = typeof(v)
            ...
            local OnCharacter = LPH_NO_VIRTUALIZE(function(character: Model)
                ...
                if Config.TheLVA.Fly.Enabled then
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while task.wait(1) do
                    if not Config.TheLVA.KillAura then -- continue end
                    if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChildOfClass("Tool") or not LocalPlayer.Character:FindFirstChildOfClass("Tool"):FindFirstChild("Setting") then -- continue end
                    for Index, Value in Players:GetPlayers() do
                        ...
                        if not DistanceCheck(Value, Config.TheLVA.KillAuraRange) then -- continue end
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while task.wait() do
                    if not Config.TheLVA.PlayerUtilities.BugPlayer then -- continue end
                    if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer) or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("HumanoidRootPart") then -- continue end                    
                    ...
                    Car_To_Use:SetPrimaryPartCFrame(Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("HumanoidRootPart").CFrame)
                end
            end))

            task.spawn(LPH_NO_VIRTUALIZE(function()
                while task.wait(.25) do
                    if not Config.TheLVA.AutoDrop then -- continue end
                    ReplicatedStorage:WaitForChild("BankProcessRemote"):InvokeServer("Drop", tostring(Config.TheLVA.MoneyAmount))
                end
            end))

            task.spawn(LPH_NO_VIRTUALIZE(function()
                while task.wait(1) do
                    if not Config.TheLVA.PlayerUtilities.AutoKill then -- continue end
                    if not LocalPlayer.Character then -- continue end
                    if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer) or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("HumanoidRootPart") then -- continue end                    
                    ...
                    if Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("Humanoid").Health == 0 then -- continue end
                    if Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChildOfClass("ForceField") then -- continue end

                    if RequireSupport then
                        kill_gun(Config.TheLVA.PlayerUtilities.SelectedPlayer, 'Head', math.huge)
                    else
                        if DistanceCheck(Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer), 300) == false then
                            local OldCFrame = LocalPlayer.Character:FindFirstChild("HumanoidRootPart").CFrame
                            Teleport(Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("HumanoidRootPart").CFrame)
                            kill_gun(Config.TheLVA.PlayerUtilities.SelectedPlayer, 'Head', math.huge)
                            task.wait(.5)
                            Teleport(OldCFrame)
                        else
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while task.wait(2) do
                    if not Config.TheLVA.PlayerUtilities.AutoRagdoll then -- continue end
                    if not LocalPlayer.Character then -- continue end
                    if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer) or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("HumanoidRootPart") then -- continue end                    
                    if not LocalPlayer.Character:FindFirstChildOfClass("Tool") then -- continue end
                    if not LocalPlayer.Character:FindFirstChildOfClass("Tool"):FindFirstChild("GunScript_Local") then -- continue end
                    ...
                    if Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("Humanoid").Health == 0 then -- continue end
                    if Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("Humanoid"):GetState() == Enum.HumanoidStateType.Physics then -- continue end

                    kill_gun(Config.TheLVA.PlayerUtilities.SelectedPlayer, 'RightUpperLeg', 0.01)
                end
            end))

            task.spawn(LPH_NO_VIRTUALIZE(function()
                while task.wait() do
                    if not Config.TheLVA.PlayerUtilities.BringingPlayer then -- continue end
                    if tostring(Config.TheLVA.PlayerUtilities.SelectedPlayer) == tostring(LocalPlayer) then -- continue end
                    if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer) or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character or not Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("HumanoidRootPart") then -- continue end
                    Players:FindFirstChild(Config.TheLVA.PlayerUtilities.SelectedPlayer).Character:FindFirstChild("HumanoidRootPart").CFrame = LocalPlayer.Character:FindFirstChild("HumanoidRootPart").CFrame + Vector3.new(2, 0, 0)
                end
            end))
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do
                    task.wait(0)
                    if Config.TheLVA.VehicleModifications.SpeedEnabled and UserInputService:IsKeyDown(Enum.KeyCode.W) then
                        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
                            if LocalPlayer.Character and typeof(LocalPlayer.Character) == "Instance" then
                                local Humanoid = LocalPlayer.Character:FindFirstChildWhichIsA("Humanoid")
                                if Humanoid and typeof(Humanoid) == "Instance" then
                                    local SeatPart = Humanoid.SeatPart
                                    if SeatPart and typeof(SeatPart) == "Instance" and SeatPart:IsA("VehicleSeat") then
                                        SeatPart.AssemblyLinearVelocity *= Vector3.new(1 + Config.TheLVA.VehicleModifications.SpeedValue, 1, 1 + Config.TheLVA.VehicleModifications.SpeedValue)
                                    end
                                end
                            end
                        end
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do
                    task.wait(0)
                    if Config.TheLVA.VehicleModifications.BreakEnabled and UserInputService:IsKeyDown(Enum.KeyCode.S) then
                        if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
                            if LocalPlayer.Character and typeof(LocalPlayer.Character) == "Instance" then
                                local Humanoid = LocalPlayer.Character:FindFirstChildWhichIsA("Humanoid")
                                if Humanoid and typeof(Humanoid) == "Instance" then
                                    local SeatPart = Humanoid.SeatPart
                                    if SeatPart and typeof(SeatPart) == "Instance" and SeatPart:IsA("VehicleSeat") then
                                        SeatPart.AssemblyLinearVelocity *= Vector3.new(1 - Config.TheLVA.VehicleModifications.BreakValue, 1, 1 - Config.TheLVA.VehicleModifications.BreakValue)
                                    end
                                end
                            end
                        end
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do task.wait()
                    if not Config.TheLVA.Farms.FarmConstructionJob then -- continue end
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do task.wait()
                    if not Config.TheLVA.Farms.FarmBank then -- continue end
                    ...
                    if not Robbable then
                        if Config.TheLVA.Farms.AFKCheck then
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do task.wait()
                    if not Config.TheLVA.Farms.FarmHouses then -- continue end
                    ...
                    if not HardDoorEnabled and Config.TheLVA.Farms.AFKCheck then
                        Teleport(SafePosition)
                        -- continue
                    end
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do task.wait()
                    if not Config.TheLVA.Farms.FarmStudio then -- continue end
            ...
            task.spawn(LPH_NO_VIRTUALIZE(function()
                while true do task.wait()
                    if not Config.TheLVA.Farms.FarmTrash then -- continue end

                    if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then -- continue end
                    if not LocalPlayer.Character:FindFirstChild("Humanoid") or LocalPlayer.Character:FindFirstChild("Humanoid").Health == 0 then -- continue end

                    for Index, Value in Workspace:GetChildren() do
                        if Value.Name == "DumpsterPromt" and Config.TheLVA.Farms.FarmTrash then
                    ...
                    if Config.TheLVA.Farms.AutoSellTrash then
                        for Index, Value in LocalPlayer.Backpack:GetChildren() do
            ...
            UserInputService.InputBegan:Connect(function(Input, GameProcessedEvent)
                if Input.KeyCode == Config.TheLVA.VehicleModifications.InstantStopBind and Config.TheLVA.VehicleModifications.InstantStop and (not GameProcessedEvent) then
                    if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid") then
                        if LocalPlayer.Character and typeof(LocalPlayer.Character) == "Instance" then
                            local Humanoid = LocalPlayer.Character:FindFirstChildWhichIsA("Humanoid")
                            if Humanoid and typeof(Humanoid) == "Instance" then
            ...
            CollectDroppedMoney = LPH_NO_VIRTUALIZE(function()  
                if not Config.TheLVA.Farms.CollectDroppedMoney then return end
            ...
            CollectLootBags = LPH_NO_VIRTUALIZE(function()  
                if not Config.TheLVA.Farms.CollectDroppedLoot then return end
                if not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then return end
                local OldCFrame = LocalPlayer.Character.HumanoidRootPart.CFrame
                ...
                for Index, Value in next, {Workspace.Storage:GetChildren()} do
                    for _Index, _Value in pairs(Value) do
                        ...
                        --[[if Config.TheLVA.Farms.OnlyCollectGuns then
            ...
            RunService.PreRender:Connect(LPH_NO_VIRTUALIZE(function()
                if not Config.TheLVA.PlayerModifications.InstantRevive then return end
                if not LocalPlayer.Character then return end
                if not LocalPlayer.Character:FindFirstChild("Humanoid") then return end
            ...
            ProximityPromptService.PromptButtonHoldBegan:Connect(function(Prompt, Self)
                if Prompt and Self == LocalPlayer and fireproximityprompt then
                    if Config.TheLVA.PlayerModifications.InstantInteract then
                        fireproximityprompt(Prompt, true)
                    end

                    if Config.TheLVA.PlayerModifications.BypassLockedCars then
            ...
            UserInputService.InputBegan:Connect(LPH_NO_VIRTUALIZE(function(Input, Game_Event)
                ...
                if Game_Event then return end
                if Input.UserInputType == Enum.UserInputType.MouseButton1 and library.flags["ClickTeleport_TheLVA"] and Config.TheLVA.ClickTeleportActive then
                    local MouseLocation = UserInputService:GetMouseLocation()
...
do
    ...
    do -- Initalize
        ...
        local ESP = function(plr)
            ...
            local Chams = Functions:Create("Highlight", {Parent = ScreenGui, FillTransparency = 1, OutlineTransparency = 0, OutlineColor = Color3.fromRGB(255, 215, 0), DepthMode = "AlwaysOnTop"})
...
-- Library functions 
    -- Misc functions
        ...
        function library:update_config_list() 
            ...
            for \1, \2 in pairs(listfiles(library.directory .. \"/configs\") do
            ...
            local items = cfg.items; do 
                ...
                -- Colorpicker
                    ...
                    items[ \"satvalpicker\" ] = library:create( \"TextButton\" , {
                        ...
                        ZIndex = 5;
                        BorderSizePixel = 0;
                        BackgroundColor3 = rgb(255, 215, 0)
                    });
                    ...
                    library:create( \"UIGradient\" , {
                        Color = rgbseq{rgbkey(0, rgb(255, 215, 0)), rgbkey(0.17, rgb(255, 255, 0)), rgbkey(0.33, rgb(0, 255, 0)), rgbkey(0.5, rgb(0, 255, 255)), rgbkey(0.67, rgb(0, 0, 255)), rgbkey(0.83, rgb(255, 0, 255)), rgbkey(1, rgb(255, 215, 0))};
                    ...
                    library:create( \"UIGradient\" , {
                        Color = rgbseq{rgbkey(0, rgb(112, 112, 112)), rgbkey(1, rgb(255, 215, 0))};
                ...
                while task.wait(0.01) do
                    ...
                    if playervalue ~= nil then
                        if table.find(library.priority, playervalue) then
                            status = \"<font color='rgb(255,215,0)'>Priority</font>\"
...
local window = library:window({name = "LVA", suffix = "", gameInfo = string.format("LVA : %s", Game_Name:lower())})
    ...
    window:seperator({name = "Game"}) do
        local LocalPlayerTab, PlayersTab, PurchaseGunTab, MiscTab, SafeTab = window:tab({name = "Main", tabs = {"Local Player", "Players", "Teleports", "Misc", "Safe"}, icon = GetImage("World.png")}) do
            do -- \\ Safe Tab
                ...
                local _SafeItem = SafeItemSection:list({flag = "SafeSelectedItem_TheLVA", options = {}, callback = function(state)
                    task.spawn(LPH_NO_VIRTUALIZE(function()
                        if not state then
                            return
                        end
                        ...
                        library.notifications:create_notification({
                            name = "LVA",
                            info = string.format("Successfully safed %s!", state),
                            lifetime = 5
                        })
                    end))
                end})
                ...
                local _TakeItem = SafeItemSection:list({flag = "TakeSelectedItem_TheLVA", options = {}, callback = function(state)
                    task.spawn(LPH_NO_VIRTUALIZE(function()
                        if not state then
                            return
                        end
                        ...
                        library.notifications:create_notification({
                            name = "LVA",
                            info = string.format("Successfully took %s from safe!", state),
                            lifetime = 5
                        })
                    end))
                end})
            ...
            do -- \\ Local Player Tab
                ...
                CharacterModsSection:toggle({type = "toggle", name = "Modify WalkSpeed", flag = "ModifyWalkSpeed_TheLVA", default = false, callback = function(state)
                    Config.MiscSettings.ModifySpeed.Enabled = state
                end})

                CharacterModsSection:toggle({type = "toggle", name = "Modify JumpPower", flag = "ModifyJumpPower_TheLVA", default = false, callback = function(state)
                    Config.MiscSettings.ModifyJump.Enabled = state
                end})

                CharacterModsSection:toggle({type = "toggle", name = "Click Teleport", flag = "ClickTeleport_TheLVA", default = false})

                local _NoClipToggle = CharacterModsSection:toggle({type = "toggle", name = "No Clip", flag = "NoClip_TheLVA", seperator = true, default = false, callback = function(state)
                    if state then
                ...
                CharacterModsSection:slider({name = "WalkSpeed Value", flag = "WalkSpeedValue_TheLVA", min = 0, max = 250, default = 50, suffix = "%", callback = function(state)
                    Config.MiscSettings.ModifySpeed.Value = state
                end})

                CharacterModsSection:slider({name = "JumpPower Value", flag = "JumpPowerValue_TheLVA", min = 0, max = 250, default = 7, suffix = "%", callback = function(state)
                    Config.MiscSettings.ModifyJump.Value = state
                end})

                CharacterModsSection:keybind({name = "Click Teleport Key", flag = "ClickTeleportKey_TheLVA", key = Enum.KeyCode.LeftControl, mode = "Hold", callback = function(state)
                    Config.TheLVA.ClickTeleportActive = state
                end})
                ...
                local _UINames, BlacklistedNames = {'ATM GUI'}, {"Dead", "Settings1", "Controls", "FirstShopGUI", "Freecam", "ThaShop2", "WATCH GUI", "NYPD Cars", "CONSTRUCTION LEVEL", "RobPlayerUI", "LVA LOCKER", 'MobileBeam', 'Settings', 'Flash', 'Enter', 'CopSirens'}
                ...
                UISection:dropdown({name = "Selected UI", flag = "SelectedUI_TB3", width = 120, items = _UINames, seperator = false, multi = false, default = 'LVA Market 2'})
            ...
            do -- \\ Players Tab
                ...
                local PlayerList = PlayerListSection:list({flag = "SelectPlayer_TheLVA", options = {}, callback = function(state)
                    Config.TheLVA.PlayerUtilities.SelectedPlayer = tostring(state)
                end})
                ...
                PlayerOptionsSection:toggle({type = "toggle", name = "Spectate Player", flag = "SpectatePlayer_TheLVA", default = false, callback = function(state)
                    Config.TheLVA.PlayerUtilities.SpectatePlayer = state
                end})

                PlayerOptionsSection:toggle({type = "toggle", name = "Bring Player", flag = "BringPlayer_TheLVA", default = false, callback = function(state)
                    Config.TheLVA.PlayerUtilities.BringingPlayer = state
                end})

                PlayerOptionsSection:toggle({type = "toggle", name = "Bug / Kill Player - Car", flag = "BugPlayer_TheLVA", default = false, callback = function(state)
                    Config.TheLVA.PlayerUtilities.BugPlayer = state
                end})

                PlayerOptionsSection:toggle({type = "toggle", name = "Auto Kill Player - Gun", flag = "AutoKillPlayer_TheLVA", default = false, callback = function(state)
                    Config.TheLVA.PlayerUtilities.AutoKill = state
                end})

                PlayerOptionsSection:toggle({type = "toggle", name = "Auto Ragdoll Player - Gun", flag = "AutoRagdollPlayer_TheLVA", seperator = true, default = false, callback = function(state)
                    Config.TheLVA.PlayerUtilities.AutoRagdoll = state
                end})

                PlayerOptionsSection:button({name = "Teleport To Player", callback = function()
                    task.spawn(Teleport, Players[Config.TheLVA.PlayerUtilities.SelectedPlayer].Character.HumanoidRootPart.CFrame)
                end})

                PlayerOptionsSection:button({name = "Down Player - Hold Gun", callback = function(state)
                    pcall(kill_gun, Config.TheLVA.PlayerUtilities.SelectedPlayer, "HumanoidRootPart", (Players[Config.TheLVA.PlayerUtilities.SelectedPlayer].Character.Humanoid.Health - 5))
                end})

                PlayerOptionsSection:button({name = "Kill Player - Hold Gun", callback = function(state)
                    pcall(kill_gun, Config.TheLVA.PlayerUtilities.SelectedPlayer, "HumanoidRootPart", math.huge)
                end})
                ...
                PlayerOptionsSection:button({name = "Fling Player - Hold Gun", callback = function(state)
                    for Index=1, 50 do
                        pcall(kill_gun, Config.TheLVA.PlayerUtilities.SelectedPlayer, "RightUpperLeg", 0.01)
                    end
                end})
            ...
            do -- \\ Misc Tab
                ...
                FarmingSection:toggle({type = "toggle", name = "Auto Farm Construction", flag = "FarmConstruction_TheLVA", default = false, callback = function(state)
                    Config.TheLVA.Farms.FarmConstructionJob = state
                end})

                FarmingSection:toggle({type = "toggle", name = "Auto Farm Bank Robbery", flag = "FarmBank_TheLVA", default = false, callback = function(state)
                    Config.TheLVA.Farms.FarmBank = state
                end})

                FarmingSection:toggle({type = "toggle", name = "Auto Farm House Robbery", flag = "FarmHouses_TheLVA", default = false, callback = function(state)
                    Config.TheLVA.Farms.FarmHouses = state
                end})

                FarmingSection:toggle({type = "toggle", name = "Auto Farm Studio Robbery", flag = "FarmStudio_TheLVA", default = false, callback = function(state)
                    Config.TheLVA.Farms.FarmStudio = state
                end})

                FarmingSection:toggle({type = "toggle", name = "Auto Farm Dumpsters", flag = "FarmDumpsters_TheLVA", default = false, callback = function(state)
                    Config.TheLVA.Farms.FarmTrash = state
                end})

                local ManualFarmSections = Column:section({name = "Manual Farms", size = 0.325, default = false, side = 'left', icon = GetImage("Pickkaxe.png")}) 

                ManualFarmSections:toggle({type = "toggle", name = "Auto Collect Dropped Cash", flag = "FarmDroppedMoney_TheLVA", default = false, callback = function(state)
                    Config.TheLVA.Farms.CollectDroppedMoney = state
                end})

                ManualFarmSections:toggle({type = "toggle", name = "Auto Collect Dropped Bags", flag = "FarmDroppedLoot_TheLVA", default = false, callback = function(state)
                    Config.TheLVA.Farms.CollectDroppedLoot = state
                end})
                ...
                ManualFarmSections:button({name = "Clean All Filthy Money", callback = LPH_NO_VIRTUALIZE(function()
                    ...
                    Teleport(CFrame.new(-223, 284, -1200))
                ...
                FarmSettingsSection:toggle({type = "toggle", name = "AFK Safety Teleport", flag = "AFKCheck_TheLVA", default = false, callback = function(state)
                    Config.TheLVA.Farms.AFKCheck = state
                end})

                FarmSettingsSection:toggle({type = "toggle", name = "Auto Sell Trash", flag = "SellTrash_TheLVA", default = false, callback = function(state)
                    Config.TheLVA.Farms.AutoSellTrash = state
                end})
                ...
                VulnerabilitySection:button({name = "Generate Max Illegal Money", callback = function()
                    ...
                    for Index, Value in pairs(Itemz) do
                ...
                KillAuraSection:slider({name = "Kill Aura Range", flag = "KillAuraRange_TB3", min = 0, max = 1000, default = 300, suffix = "st", callback = function(state)
                    Config.TheLVA.KillAuraRange = state
                end})
            end
            ...
            do -- \\ Purchase Guns
                ...
                WeaponListSection:list({flag = "PurchaseSelectedItem_TheLVA", options = Config.Guns, callback = function(state)
                    task.spawn(LPH_NO_VIRTUALIZE(function()
                        if not state then
                            return
                        end

                        Config.TheLVA.Selected_Item = state

                        local self = string.match(Config.TheLVA.Selected_Item, "^(.*) %-");
                        ...
                        library.notifications:create_notification({
                            name = "LVA",
                            info = string.format("Successfully purchased %s!", self),
                            lifetime = 5
                        })
                    end))
                end})
                ...
                for Index, Value in Config.TheLVA.TeleportationList do
                    table.insert(List, Index)
                end

                table.sort(List)

                TeleportListSection:list({flag = "TeleportToPlace_TheLVA", options = List, callback = function(state)
                    task.spawn(LPH_NO_VIRTUALIZE(function()
                        if not state then
                            return
                        end

                        Teleport(Config.TheLVA.TeleportationList[state])

                        library.notifications:create_notification({
                            name = "LVA",
                            info = string.format("Successfully teleported to %s!", state),
                            lifetime = 5
                        })
                    end))
                end})
    ...
    window:seperator({name = "Game"}) do
        local LocalPlayerTab, PlayersTab, PurchaseGunTab = window:tab({name = "Main", tabs = {"Local Player", "Players", "Teleports"}, icon = GetImage("World.png")}) do
            do -- \\ Local Player
                ...
                for _, Index in pairs(__Modifications) do
                    LocalPlayerModsSection:toggle({type = "toggle", name = Index, flag = Index.."_SB", default = false, callback = Index ~= "No Clip" and LPH_NO_VIRTUALIZE(function(Value)
                            ...
                            Config.South_LVA.LocalPlayer_Config[Index] = Value
                ...
                LocalPlayerModsSection:slider({name = "WalkSpeed Value", flag = "WalkSpeedValue_SouthLVA", min = 0, max = 50, default = 25, suffix = "%", callback = function(state)
                    Config.South_LVA.LocalPlayer_Config.SpeedValue = state/100
                end})

                LocalPlayerModsSection:keybind({name = "Delete + Click Key", flag = "DeleteOnKey_SouthLVA", key = Enum.KeyCode.LeftControl, mode = "Hold", callback = function(state)
                    Config.South_LVA.LocalPlayer_Config.DeleteKey = library.flags["DeleteOnKey_SouthLVA"].key
                end})
                ...
                TeleportMethodSection:dropdown({name = "Select Method", flag = "TeleportMethod_SB", width = 100, items = {"Dirt Bike", "Damage", "Tween"}, seperator = false, multi = false, default = "Damage", callback = function(state)
                    Config.South_LVA.TeleportMethod = state
                end})
                ...
                VulnSection:toggle({name = "Free Tier 1, 2 and 3", default = false, flag = "Free_Tiers", type = 'toggle', callback = function(state)
                    ...
                    for Index, Value in pairs(Tiers) do
                ...
                FarmSection:toggle({name = "Auto-Farm Cards", default = false, flag = "Card_Auto_Farm", type = "toggle", callback = function(state)
                    task.spawn(function() if not _ScriptLoaded then return end
                        if Config.South_LVA.OwnedBike == "Unknown" then
                            local Bike = Find_Bike()
                            if Bike == nil then
                                ReplicatedStorage:WaitForChild("RemoteEvents"):WaitForChild("Dealershipinteraction"):FireServer("Spawn", "DirtBike")
                                task.wait(1)
                                Bike = Find_Bike()
                                task.wait(1)
                                if Bike == nil then Config.South_LVA.OwnedBike = "No" else Config.South_LVA.OwnedBike = "Yes" end
                            else
                                Config.South_LVA.OwnedBike = "Yes"
                            end
                            task.wait(0.5)
                        end
                        Config.South_LVA.FarmingUtilities.CardFarm = state
                ...
                FarmSection:toggle({name = "Auto-Farm Boxes", default = false, flag = "Box_Auto_Farm", type = "toggle", callback = function(state)
                    if not _ScriptLoaded then return end
                    Config.South_LVA.FarmingUtilities.BoxFarm = state
                ...
                FarmSection:toggle({name = "Auto-Farm Chips", default = false, flag = "Chip_Auto_Farm", type = "toggle", callback = function(state)
                    task.spawn(function() if not _ScriptLoaded then return end
                        ...
                        Config.South_LVA.FarmingUtilities.ChipFarm = state
                ...
                FarmSection:toggle({name = "Auto-Farm Marshmallows", default = false, flag = "Marshmallow_Auto_Farm", type = "toggle", callback = function(state)
                    task.spawn(function() if not _ScriptLoaded then return end
                        ...
                        Config.South_LVA.FarmingUtilities.MarshmallowFarm = state
                ...
                _MarshMallowDropdown = FarmSection:slider({name = "Marshmallow Amount - $950", flag = "Marshmallow_Amount", min = 1, max = 50, default = 5, suffix = "", callback = function(state)
                    Config.South_LVA.FarmingUtilities.MarshmallowIncrement = state
            ...
            do -- \\ Teleports
                ...
                for Index, Value in Config.South_LVA.Locations do
                ...
                WeaponListSection:list({flag = "PurchaseSelectedItem_SouthLVA", options = Config.South_LVA.Guns, callback = function(v)
                    task.spawn(LPH_NO_VIRTUALIZE(function()
                        if not v then return end

                        Config.South_LVA.Selected_Item = tostring(v)

                        local self = string.match(Config.South_LVA.Selected_Item, "^(.*) %-");
                        ...
                        local suc, err = pcall(function()
                            ...
                            if Teleport_Status == "Failed" then
                                library.notifications:create_notification({
                                    name = "LVA",
                                    info = string.format("Failed to purchase %s!", self),
                                    lifetime = 7.5
                                })
                            ...
                            for Index = 1, Config.South_LVA.Item_Amount do
                        ...
                        if not DidntBuy then
                            if suc then
                                ...
                                library.notifications:create_notification({
                                    name = "LVA",
                                    info = string.format("Failed to purchase item %s . error : %s", self, err),
                                    lifetime = 15
                                })
                            end
                        end
                ...
                TeleportListSection:list({flag = "TeleportToPlace_SouthLVA", options = Location_Names, callback = function(state)
                    task.spawn(LPH_NO_VIRTUALIZE(function()
                        if not state then
                            return
                        end
                        ...
                        Config.South_LVA.Selected_Location = state
                        ...
                        local suc, error = pcall(function()
                            if Config.South_LVA.Selected_Location ~= "Dirty Hobo 💩" and Config.South_LVA.Selected_Location ~= "Active ATM 🏧" then
                            _Position = Config.South_LVA.Locations[Config.South_LVA.Selected_Location]
                            Teleport(Config.South_LVA.Locations[Config.South_LVA.Selected_Location])
                            elseif Config.South_LVA.Selected_Location == "Dirty Hobo 💩" then
                            ...
                            elseif Config.South_LVA.Selected_Location == "Active ATM 🏧" then
                                local ATMPositions = {
                                    ATM1 = CFrame.new(-30, 4, -300);
                                    ATM2 = CFrame.new(539, 4, -353);
                                    ATM3 = CFrame.new(497, 4, 403);
                        ...
                        if (LocalPlayer.Character.HumanoidRootPart.Position - _Position.Position).Magnitude > 20 then
                            library.notifications:create_notification({
                                name = "LVA",
                                info = string.format("Failed teleported to %s!", state),
                                lifetime = 7.5
                            })
                        ...
                        if suc then
                            ...
                            library.notifications:create_notification({
                                name = "LVA",
                                info = string.format("Teleportation to %s. error : %s", state, err),
                                lifetime = 15
                            })
                        end
                    end))
            ...
            do -- \\ Player Tab        
                ...
                local PlayerList = PlayerListSection:list({flag = "SelectPlayer_SouthLVA", options = {}, callback = function(state)
                    Config.South_LVA.PlayerUtilities.SelectedPlayer = tostring(state)
                end})
                ...
                PlayerOptionsSection:toggle({type = "toggle", name = "Spectate Player", flag = "SpectatePlayer_SouthLVA", default = false, callback = function(state)
                    Config.South_LVA.PlayerUtilities.SpectatePlayer = state
                end})

                PlayerOptionsSection:toggle({type = "toggle", name = "Bring Player", flag = "BringPlayer_SouthLVA", default = false, callback = function(state)
                    Config.South_LVA.PlayerUtilities.BringingPlayer = state
                end})

                PlayerOptionsSection:button({name = "Teleport To Player", callback = function()
                    task.spawn(function()
                        if not Config.South_LVA.PlayerUtilities.SelectedPlayer then return end

                        local Success, Error = pcall(function()
                            Teleport(Players[Config.South_LVA.PlayerUtilities.SelectedPlayer].Character.HumanoidRootPart.CFrame)
                        end)

                        if (LocalPlayer.Character.HumanoidRootPart.Position - Players[Config.South_LVA.PlayerUtilities.SelectedPlayer].Character.HumanoidRootPart.Position).Magnitude > 20 then
                            library.notifications:create_notification({
                                name = "LVA",
                                info = "Failed to teleport to {Config.South_LVA.PlayerUtilities.SelectedPlayer}!",
                                lifetime = 7.5
                            })
                        ...
                        if Success then
                            library.notifications:create_notification({
                                name = "LVA",
                                info = "Successfully teleported to {Config.South_LVA.PlayerUtilities.SelectedPlayer}!",
                                lifetime = 7.5
                            })
                        else
                            library.notifications:create_notification({
                                name = "LVA",
                                info = string.format("Failed to teleport to {Config.South_LVA.PlayerUtilities.SelectedPlayer}. Error : %s", Error),
                                lifetime = 10
                            })
                        end
                    end)
                ...
                PlayerOptionsSection:button({name = "Get Into Players Car", callback = function()
                    pcall(SitInPlayersVehicle, Players[Config.South_LVA.PlayerUtilities.SelectedPlayer])
                end})
            end
        end
    end
...
window:seperator({name = "Combat"}) do
    local SilentAimTab = window:tab({name = "Silent Aim", tabs = {"General Settings"}, icon = GetImage("Pistol.png")}) do
        ...
        SettingsSection:slider({name = "Max Distance", flag = "MaxDistance_Silent", min = 0, max = (Game_Name == "South LVA") and 300 or 3000, default = (Game_Name == "South LVA") and 300 or 1000, suffix = "st", callback = function(state)
    ...
    local AimlockTab = window:tab({name = "Aimlock", tabs = {"General Settings"}, icon = GetImage("Aimlock.png")}) do
        ...
        SettingsSection:slider({name = "Max Distance", flag = "MaxDistance_Aimlock", min = 0, max = 3000, default = ((Game_Name == "South LVA") and 300 or 1000), suffix = "st", callback = function(state)
    ...
    if Game_Name == "The LVA" and not Solara then
        local WeaponModTab, MiscModTab = window:tab({name = "Modifications", tabs = {"Weapon Modifications", "Hitbox Modifications"}, icon = GetImage("Wrench.png")}) do
            local WeaponModTabColumn = WeaponModTab:column({}) do
                ...
                for _, Index in pairs(Modifications) do
                    GeneralSection:toggle({name = Index, flag = Index.."_TB3", type = "toggle", default = false, callback = function(state)
                        ...
                        Config.TheLVA.Modifications[Index:gsub(" ", "")] = state
                    end})
                end
                ...
                GeneralSection:slider({name = "Recoil Percentage", flag = "RecoilValue_TB3", default = 50, min = 0, max = 100, suffix = "%", callback = function(state)
                    Config.TheLVA.Modifications.RecoilPercentage = state
                end})
                ...
                GeneralSection:slider({name = "Spread Percentage", flag = "SpreadValue_TB3", default = 50, min = 0, max = 100, suffix = "%", callback = function(state)
                    Config.TheLVA.Modifications.SpreadPercentage = state
                end})
                ...
                GeneralSection:slider({name = "Fire Rate Percentage", flag = "FireRateSpeed_TB3", default = 50, min = 0, max = 100, suffix = "%", callback = function(state)
                    Config.TheLVA.Modifications.FireRateSpeed = state
                end})
                ...
                GeneralSection:slider({name = "Reload Speed Percentage", flag = "ReloadSpeed_TB3", default = 50, min = 0, max = 100, suffix = "%", callback = function(state)
                    Config.TheLVA.Modifications.ReloadSpeed = state
                end})
                ...
                GeneralSection:slider({name = "Equip Speed Percentage", flag = "EquipSpeed_TB3", default = 50, min = 0, max = 100, suffix = "%", callback = function(state)
                    Config.TheLVA.Modifications.EquipSpeed = state
                end})
            end
        ...
        local WeaponModTab = window:tab({name = "Modifications", tabs = {"Weapon Modifications"}, icon = GetImage("Wrench.png")}) do
            local WeaponModTabColumn = WeaponModTab:column({}) do
                ...
                for _, Index in pairs(Modifications) do
                    GeneralSection:toggle({name = Index, flag = Index.."_TB3", type = "toggle", default = false, callback = function(state)
                        if Index == "Fully Automatic" then Index = "Automatic" end
                        Config.South_LVA.Modifications[Index:gsub(" ", "")] = state
                    end})
                end
                ...
                GeneralSection:slider({name = "Recoil Percentage", flag = "RecoilValue_TB3", default = 50, min = 0, max = 100, suffix = "%", callback = function(state)
                    Config.South_LVA.Modifications.RecoilPercentage = state
                end})

                GeneralSection:slider({name = "Spread Percentage", flag = "SpreadValue_TB3", default = 50, min = 0, max = 100, suffix = "%", callback = function(state)
                    Config.South_LVA.Modifications.SpreadPercentage = state
                end})

                GeneralSection:slider({name = "Fire Rate Percentage", flag = "FireRateSpeed_TB3", default = 50, min = 0, max = 100, suffix = "%", callback = function(state)
                    Config.South_LVA.Modifications.FireRateSpeed = state
                end})

                GeneralSection:slider({name = "Reload Speed Percentage", flag = "ReloadSpeed_TB3", default = 50, min = 0, max = 100, suffix = "%", callback = function(state)
                    Config.South_LVA.Modifications.ReloadSpeed = state
                end})

                GeneralSection:slider({name = "Equip Speed Percentage", flag = "EquipSpeed_TB3", default = 50, min = 0, max = 100, suffix = "%", callback = function(state)
                    Config.South_LVA.Modifications.EquipSpeed = state
                end})
            end
        end
    end
...
window:seperator({name = "World"}) do
    ...
    _FilledBoxes:colorpicker({flag = "PlayerVisuals_FilledBoxes_Color1", color = Color3.fromRGB(255, 215, 0), alpha = 0.25, callback = function(state, alpha)
    ...
    _ChamsToggle:colorpicker({flag = "PlayerVisuals_Chams_Color1", color = Color3.fromRGB(255, 215, 0), alpha = 0.8, callback = function(state, alpha)
...
if Game_Name == "South LVA" then
    local DupeTab = window:tab({name = "Duplication", tabs = {"General"}, icon = GetImage("Cash.png")}) do
        ...
        DuplicationSection:textbox({name = "Selected Player's Name", callback = function(text)
            ...
            writefile("SouthLVAUsernameRealGame.txt", name)
        end})

        DuplicationSection:textbox({name = "Amount To Send (Max Is $20000)", callback = function(text)
            writefile("SouthLVAAmountRealGame.txt", text)
        end})
...
if hookfunction and not Solara and LPH_OBFUSCATED and Game_Name == "South LVA" then
    local _FireServer;
    local _Function;
